...
export function SlashdotModule({ envelope }: ModuleDisplayProps<unknown, SlashdotModuleData>) {
  ...
  return (
    <ul className="flex flex-col gap-2 overflow-y-auto">
      {items.map((item) => (
        <li key={item.url} className="border-b border-slate-800 pb-2 last:border-0">
          <a
            href={item.url}
            target="_blank"
            rel="noreferrer"
