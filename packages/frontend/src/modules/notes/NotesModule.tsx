...
export function NotesModule({ instance }: ModuleDisplayProps<NotesConfig, unknown>) {
  ...
  return (
    <textarea
      ...
      rows={12}
      className="w-full flex-1 resize-y rounded border border-slate-800 bg-slate-950 p-2 text-sm text-slate-200 placeholder:text-slate-500 focus:border-slate-600 focus:outline-none"
    />
  );
}
